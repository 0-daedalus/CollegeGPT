        ...
        <>
          ...
          <Layout>
            <div className="h-full w-full p-8 flex justify-center">
              <div className="flex flex-col justify-center items-center gap-4 w-full md:w-1/2 bg-zinc-50 rounded-xl shadow-lg p-8">
                {uni && !roadmap ? (
                  <>
                    <Image
                      priority
                      src={uni.imageUrl}
                      className="w-fit h-48 md:h-1/2"
                      height={400}
                      width={400}
                      unoptimized
                      alt={`${uni.name} photo`}
                    ...
                    <button
                      type="button"
                      className="bg-orange-500 rounded-md text-white h-fit text-lg p-4 mt-8 md:text-base"
                      onClick={() => setClicked(true)}
                    >
                      {clicked ? "Loading..." : "Generate Roadmap"}
                    </button>
                  </>
                ) : roadmap ? (
                  <>
